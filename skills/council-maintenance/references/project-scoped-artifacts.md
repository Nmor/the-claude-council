# council-maintenance: project-scoped-artifacts.md — full text

> Full text of the hard rules + scaffold + tech-stack detection of
> `~/.claude/rules/common/project-scoped-artifacts.md`, whose always-on Floor copy carries only
> their summary. Pointed at by the `## Full rule text carried here` row in `../SKILL.md`.
>
...
### 9. A plan belongs to a project by name

Plan mode writes to `~/.claude/plans/` unless `plansDirectory` is set, and that setting resolves
relative to the repository the session starts in (settings reference, read 2026-09-21). So several
projects' plans share that folder, and a project spanning several repositories cannot give them one
plans folder natively. What binds a plan to a project is the project's memory: one
`Active plan: <path>` line in its `MEMORY.md` (`project-memory.md` rule 4). The gates read that
line and never guess from a shared folder. A plan may also live in `<workspace>/.claude/plans/`;
the pointer is what makes it this project's. A project has exactly one plan
(`one-plan-per-workspace.md`): a shared-folder file its pointer does not name is a draft to merge
into that plan, never a second plan beside it.

### 10. Project memory is the memory Claude Code loads for the project

Per `project-memory.md` rule 1: auto memory at `~/.claude/projects/<project>/memory/`, keyed by
the repository the session starts in, moved with `autoMemoryDirectory`. A workspace
`.claude/memory/` is loaded only when `autoMemoryDirectory` points at it. Preferences meant for
every project belong in user-level instructions (`~/.claude/CLAUDE.md` or `~/.claude/rules/`),
which every session loads; a memory directory is loaded for one project only.
