# council-rules: model-tier-selection.md — full text

> Full text of model-tier-selection: tier capability table, the ladder resolution algorithm,
> availability config, graceful degradation, alias currency and anti-patterns. Pointed at by the
> SKILL.md routing row "Model-tier ladders + availability".
>
> **Size budget: 12 KB** — `token-budget.mjs --check`.

## `model-tier-selection.md` — full text
...
- If a resolved `M` spawn fails as **model-unavailable** (overloaded, timed out,
  rate-limited), **drop `M` from the available set for this session and take the
  next more capable rung that is still available** — up the ladder, not down. An
  outage is absorbed by cost, never by quality: a `sonnet → haiku` role whose
  Sonnet rung times out runs on Opus. Surface a one-line note (per
  `no-silent-failures.md`).
- If a Fable call returns `stop_reason: "refusal"` with no fallback configured,
  the next rung down (Opus) is the right one: the refusal is about the content
  class, and Opus is the model Anthropic itself falls back to.
- Only when nothing above `M` is available does the walk go down; never hard-fail
  while a floor model is still available, and never succeed on a lower tier
  without saying so.
- Plan limits ("You've hit your Opus limit") are NOT covered by `fallbackModel`, which
  skips rate-limit and billing errors (docs `model-config`, read 2026-09-25). The
  `model-exhaustion-marker.js` hook (StopFailure, Stop, PostToolUse and
  PostToolUseFailure on Agent) records the named tier in `.local/model-exhausted.json`
  for one 5-hour window, or until that tier next succeeds. `model-ladder-gate.js`
  removes it from the available set: the ladder falls up, and a spawn that asks for the
  exhausted tier is denied with the rung to use. The session model itself cannot be
  switched by any hook or setting; that one step is `/model`.
- IDE side, from the Claude Code docs (`model-config`, read 2026-09-22):
  `fallbackModel` is an ordered chain tried when the primary is overloaded or
  unavailable, for the current turn only; set it to `["opus", "sonnet"]` so a
  Fable session falls up-to-nearest, not to Haiku. The auto-mode classifier
  runs on Sonnet 5 by default and has no setting of its own; it runs on the
  session's model (Opus under Fable) only when `availableModels` excludes
  Sonnet, which also removes Sonnet from every ladder, so that trade is the
  owner's call. A classifier timeout ("cannot determine the safety of an
  action") is transient: retry the action, do not route around the check.
