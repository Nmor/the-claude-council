...
> The full per-division trigger catalog: file globs, keyword sets, change-scope triggers and veto
> authority for every Extended Division, plus cross-cutting composition and trigger evaluation.
> Pointed at by the SKILL.md routing row "Full per-division trigger catalog".
>
...
- Division engaged but its trigger ruleset didn't fire (false-positive engagement — trigger needs
  narrowing)
...
- Multiple Divisions fire on the same task but one consistently has nothing material to add
  (right-sizing)
...
- Tightening of "auto-fire" logic when a Division consistently engages on edge cases that don't need
  it
