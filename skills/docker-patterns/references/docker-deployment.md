...
> Covers **docker-deployment (migrated rule)** for the `docker-patterns` skill. Routed from the
> reference map in `../SKILL.md`.
>
> Auto-activates for Dockerfiles, compose files, CI/CD workflows, and K8s manifests. Chains with
> `docker-patterns` and `deployment-patterns` skills.
>
...
- Secrets baked into image layer (`ENV API_KEY=...`, COPY of `.env`) — sister
  `secrets-management.md` weakening
...
- Base image tagged `latest` / `master` / `edge` instead of pinned + digest (sister
  `dependency-pinning.md` weakening)
...
- New row in the Docker checklist when a recurring image-bloat / supply-chain class emerges (e.g.,
  missing `SBOM` generation, missing `LABEL` metadata)
- Tightening of the deployment checklist when a recurring rollout failure class recurs (e.g., DB
  migration races, feature-flag desync)
- New cross-reference when a sister rule (docker-localhost-binding, deploy-failures-become-checks,
  github-actions-gotchas) provides a deploy-time gate
