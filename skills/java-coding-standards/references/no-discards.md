...
> Covers the migrated `rules-library/java/no-discards.md` rule body — the 15 banned patterns (empty
> catch through discarded `CompletableFuture`), required tooling, verification block. Pointed at by
> the `No-discards (banned patterns)` row of `../SKILL.md`.
>
