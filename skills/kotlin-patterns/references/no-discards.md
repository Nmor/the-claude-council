...
> Covers the Kotlin no-discards extension: the 13 banned patterns (force-unwrap, broad catch, silent
> `runCatching`, unjustified `@Suppress`, `println`, mutable top-level state, `lateinit`, `Any`
> widening, missing value semantics, ignored `Job`/`Deferred`, `GlobalScope.launch`, `Thread.sleep`
> in suspend, `Random()`), the strict `detekt.yml` baseline, and the verification block. Pointed at
> by the **No-discards + detekt baseline** row in ../SKILL.md.
>
