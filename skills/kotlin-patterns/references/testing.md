...
> Covers Kotlin testing: the test pyramid, JUnit 5 + MockK and Kotest idioms, parameterised tests,
> coroutine testing with `runTest`, Testcontainers, property tests, Compose UI tests, and the five
> hard rules (no flaky time, clock/RNG injection, coverage gates, behavioural naming, no
> `runBlocking`). Pointed at by the **Testing** row in ../SKILL.md.
>
