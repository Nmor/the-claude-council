...
Covers the `.githooks/pre-commit` and `pre-push` gates, the multi-version Lua CI workflow, the
strict `.luacheckrc` and `stylua.toml` baselines, and the OpenResty + Neovim specific commands.
The detail behind the SKILL.md row **Hooks + CI**.
