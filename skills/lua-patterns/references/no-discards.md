...
Covers the eight banned patterns — ignored multi-return, unprotected calls, globals, nil access,
empty error handlers, `print()`, `load` on user input, missing error-path return — plus the
required `.luacheckrc` and the sweep verification block. The detail behind the SKILL.md row
**No-discards (banned patterns)**.
