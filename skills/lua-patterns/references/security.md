...
Covers code injection via `load`, untrusted bytecode, `_ENV` sandboxing, `io.*` / `os.*` exposure,
`require` control, randomness, the Redis / OpenResty / Neovim / game-engine embedding threat
models, secrets handling and required tooling. The detail behind the SKILL.md row **Security**.
