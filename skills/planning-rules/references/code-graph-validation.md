...
> Covers the code-graph validation rule in full — incremental per-task checks, the per-surface and
> per-language command matrices, the phase-boundary and pre-push sweeps, anti-patterns,
> cross-references and learning hooks. Pointed at by the **Code-graph validation** row of
> `../SKILL.md`.
>
