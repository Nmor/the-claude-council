...
> The fifteen banned Ruby patterns — bare `rescue`, `rescue nil`, `send` with user input, `eval`,
> suppressions, unsynchronised `Hash#[]=`, `puts`, `Time.now`, deep `ENV`, monkey-patching,
> unordered `find_each`, callback-skipping updates, `for ... in`, discarded multiple assignment —
> plus the required tooling and the verification block. Routed from the **No-discards (Ruby)** row
> of `../SKILL.md`.
>
