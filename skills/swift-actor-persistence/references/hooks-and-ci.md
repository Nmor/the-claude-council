...
> Covers Swift hooks + CI + tooling config: pre-commit / pre-push gates, the GitHub Actions
> workflow, required tools, `.swiftformat`, the strict `Package.swift` baseline, Xcode `.xcconfig`
> hardening, SwiftPM dependency hygiene and the pre-deployment iOS checklist. Pointed at by the
> SKILL.md routing row **Hooks, CI + tooling config**.
>
