...
> Covers the Swift no-discards extension: force-unwrap, force-try, empty catch, ignored results,
> `@discardableResult`, implicitly unwrapped optionals, `Any` returns, `print`, unremoved observers,
> retain cycles, and the required `.swiftlint.yml`. Pointed at by the SKILL.md routing row
> **No-discards — banned patterns + SwiftLint config**.
>
