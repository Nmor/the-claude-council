...
> Covers Swift patterns: project layout, value types, `Sendable` + `actor`, structured concurrency,
> protocols with associated types, `Result`, SwiftUI, dependency injection, Combine vs
> AsyncSequence, reuse-first library choices and the anti-pattern table. Pointed at by the SKILL.md
> routing row **Swift patterns — architecture + concurrency + SwiftUI**.
>
