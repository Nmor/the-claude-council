...
- `error-shape-contract-testing` — pin response shapes so
  backend / frontend don't silently drift apart.
- `fire-and-forget` — canonical shape for side effects that don't
  block the user but must still log on failure.
