...
> Covers **a11y (migrated rule)** for the `wcag-accessibility` skill. Routed from the reference map
> in `../SKILL.md`.
>
...
- WCAG 2.2 NEW success criterion (2.4.11 / 2.5.7 / 2.5.8 / 3.2.6 / 3.3.7 / 3.3.8) missed on a UI
  surface (rule 1 weakening)
...
- Tightening of the AAA list when a recurring regulatory finding suggests AA is insufficient for a
  path
- New cross-reference when a sister rule (i18n, frontend-patterns) prescribes a complementary a11y
  constraint
