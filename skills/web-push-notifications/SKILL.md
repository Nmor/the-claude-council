...
self.addEventListener("pushsubscriptionchange", (event) => {
  event.waitUntil((async () => {
    ...
    for (const client of clients) {
      client.postMessage({ type: "stewardbot:push-resubscribe", subscription: sub.toJSON() });
    }
  })());
});
...
navigator.serviceWorker.addEventListener("message", async (event) => {
  if (event.data?.type !== "stewardbot:push-resubscribe") return;
...
self.addEventListener("notificationclick", (event) => {
  ...
  event.waitUntil((async () => {
    ...
    if (clients.length > 0) {
      ...
      clients[0].postMessage({ type: "stewardbot:notification-click", target });
      return;
    }
    await self.clients.openWindow(target ?? "/calendar");
  })());
});

// useWebPushRegister.ts — page handler with router
navigator.serviceWorker.addEventListener("message", (event) => {
  if (event.data?.type !== "stewardbot:notification-click") return;
