...
Covers YAML 1.2.2 syntax discipline — indentation, quoting, block vs flow, document markers, line
length, comments, anchors, schema references, key casing, per-ecosystem checklists, and the strict
`.yamllint.yaml` baseline. Pointed at by the SKILL.md routing row "Coding style".
