...
Covers YAML as attack surface — safe parsers, secret handling, SHA-pinning, RBAC + pod security +
network policy, Actions permissions, schema validation, billion-laughs, provenance signing,
per-ecosystem security checklists, and required tooling. Pointed at by the SKILL.md routing row
"Security".
